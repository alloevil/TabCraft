...
> **Chrome 版本要求**：扩展以 Chrome 120+ 为构建目标；本地 AI（Gemini Nano）需 Chrome 138+，详见[第 7 节](#7-启用本地-aigemini-nano)。未满足时会**自动回退到规则引擎**，功能不受影响。
...
| **稍后提醒 Snooze** ⏰   | 关闭整个分组，但会先把每个标签页保存到本地（`snoozed`），不会静默丢失 | 分组卡片上的 ⏰ 按钮                          |
| **规则 Rules** 📋        | 查看/增删改**自定义**域名规则（叠加在内置的 390 条规则之上）          | Rules 视图                                    |
| **统计 Stats** 📊        | 累计分组数、休眠数、关闭的重复数、预估省下的内存                      | Stats 视图                                    |
| **代理指示器** 🛰️        | 在每个网页角落显示这页流量实际走的代理节点（需手动开启）              | Settings 视图 → Proxy indicator               |
...
| **Show Proxy On Every Page**（网页代理指示器） | 关                      | 在每个网页角落显示一个小标签，写明这个页面的流量实际走了哪个代理出口。开启时会向你申请"访问所有网站"权限（仅此功能需要），关闭时自动归还                                                                                                |
| **Controller API**（控制器地址）               | `http://127.0.0.1:9097` | Clash / mihomo 的 `external-controller` 地址。可以只填 `127.0.0.1:9090`，会自动补上 `http://`                                                                                                                                           |
| **API Secret**（API 密钥）                     | 空                      | 控制器设了 `secret` 时填这里，以 `Authorization: Bearer` 发送。只存在本地，不同步                                                                                                                                                       |
| **Position**（显示位置）                       | 右下                    | 标签停靠的角落：左上 / 右上 / 左下 / 右下                                                                                                                                                                                               |
...
标签上显示的是出口节点名。鼠标悬停展开完整链路：页面主机名、节点全名、经过的策略组、命中的规则（例如 `matched DomainKeyword(github)`）。点一下标签可以在本次浏览中把它收起来。

- **实心圆点** = 这条路由是该主机自己的连接测到的。
- **空心圆点** = 没抓到该主机的连接，是用同一注册域名下的相邻主机推断的，悬停详情里会写明"据 xxx 推断"。
- 显示 **直连 / Direct** 说明这个站点命中了直连规则，根本没走代理；**已拦截 / Blocked** 说明命中了 REJECT 规则。
...
**模型还在下载的时候呢？** TabCraft 对 Prompt API 的两次探测各有超时（可用性 3 秒、建会话 10 秒），超时就先按"没有本地 AI"处理、走规则引擎，等下次 Service Worker 启动再试。这样即使模型迟迟不就绪，扩展的其余功能也不会被拖住。
...
**Q：数据存在哪？会同步吗？**
A：规则、设置、统计都存在 `chrome.storage.local`，仅本机，不随账号同步、不上传服务器。可在 Settings 视图导出/导入整库备份。
