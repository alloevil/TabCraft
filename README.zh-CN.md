...
大多数标签页管理器只会按域名分组。TabCraft 会阅读页面标题和 URL 路径，理解每个标签页**实际上在讲什么**。一个标题为 "Investment Dashboard" 的 localhost 页面会被归入**金融**分组，而不是**开发**分组。
...
| **🧠 自我学习**      | 从你的手动分组和 AI 高置信判定中学习域名→分组的映射（可选开启）                                 |
| **📋 域名规则**      | 自定义规则叠加在内置的 390 条规则之上——可自行增删改（随 Settings 的数据导出/导入一起备份）      |
| **🔍 重复检测**      | 智能 URL 匹配，自动忽略跟踪参数                                                                 |
| **💤 标签页休眠**    | 自动挂起不活跃的标签页以释放内存（统计页按每个休眠标签页约 50 MB 估算，仅为粗略估计，并非实测） |
| **🗂️ 工作区**        | 保存并恢复带名字的标签页快照                                                                    |
| **🎨 侧边栏界面**    | 现代玻璃拟态（glassmorphism）界面，支持深色/浅色模式                                            |
| **🛰️ 代理指示器**    | 在每个网页显示该页流量实际走的代理出口节点（需手动开启）                                        |
| **🔒 100% 隐私保护** | 所有分类均在本地完成；唯一的对外请求是你在设置里填写的代理控制器地址（默认走本机回环）          |
...
- Node.js 22+（`engines` 要求 `^22.22.2 || ^24.15.0 || >=26`；CI 在 22 与 24 上构建；`.nvmrc` 固定推荐版本）。Node 20 已于 2026-04-30 结束支持，测试工具链不再兼容
- Chrome 120+（扩展的构建目标；AI 功能需要 Chrome 138+，Prompt API 从该版本起对扩展可用）
...
| **存储** | chrome.storage.local（代理路由的临时缓存另用 chrome.storage.session） |
...
│   ├── proxy-badge.ts   # 代理角标注入（可选开启）
│   ├── proxy-monitor.ts # Clash/mihomo 控制器轮询
│   └── storage.ts       # 数据持久化
├── sidepanel/           # UI 面板
│   ├── components/      # React 组件
